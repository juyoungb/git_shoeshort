...
<c:forEach items="${oi.getDetailList()}" var="dl">
    ...
    <tr>
        <td width="150px;"><img src="resources/img/product/${dl.getOd_img()}" style="display: block; width: 100%; margin: 2px auto; cursor: pointer; " 
    			/></td>
        <td>${dl.getOd_name()} (사이즈:${dl.getOd_size()})</td>
        <td>${dl.getOd_cnt()}</td>
        <td><fmt:formatNumber value="${dl.getOd_price() * dl.getOd_cnt()}" pattern="#,###원"/></td>
        <td><fmt:formatNumber value="${dl.getOd_price() * dl.getOd_cnt() * 0.002}" pattern="#,###P"/></td>
    </tr>
</c:forEach>

</table><br>
<table style=" border-collapse:collapse;"><tr>
<th width="85%" style="text-align:right;">총 적립 포인트</th><td style="text-align:right;"><fmt:formatNumber value="${tpoint}" pattern="#,###P"/></td>
</tr><tr>
<th width="85%" style="text-align:right;">총 결제 금액</th><td style="text-align:right;"><fmt:formatNumber value="${tprice}" pattern="#,###원"/></td>
</tr>
</table>
